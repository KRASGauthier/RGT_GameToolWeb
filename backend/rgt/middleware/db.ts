...
export const appDB: IDB = {
	main: mongoose.createConnection(
		`mongodb://${process.env.DB_USERNAME}:${process.env.DB_PASSWORD}@mongo:27017/${process.env.DB_NAME}?authSource=admin`,
	),
	users: mongoose.createConnection(
		`mongodb://${process.env.DB_USERNAME}:${process.env.DB_PASSWORD}@mongo:27017/${process.env.DB_USERS_NAME}?authSource=admin`,
	),
};

export const checkMongoDB = () => {
	(Object.entries(appDB) as [keyof IDB, Connection][]).forEach(
		([key, db]: [keyof IDB, Connection]) => {
			db.asPromise()
				.then(() => {
