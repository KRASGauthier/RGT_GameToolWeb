...
function CToggle({label, entries, value, borderRadius, borderColor, styling, checkedStyling, onChange}: CToggleProps) {
	...
	return <Stack direction={"column"}>
		...
		{entries.map((entry: IToggleEntry, index: number) => {
			return <CButtonIconText
				...
				key={entry.value} 
				styling={styling} checkedStyling={checkedStyling}
				onClick={() => {onChange?.(entry.value)}} checked={entry.value == value}>{entry.display ?? entry.value}</CButtonIconText>
		})}
		</Stack>
	</Stack>
}
