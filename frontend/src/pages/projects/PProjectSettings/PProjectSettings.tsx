...
function PProjectSettings({}: PProjectSettingsProps) {
	...
	if(!project)
		return;
	return <Stack sx={{flex: 1}} direction={"row"} >
		<Stack sx={{flex: 1, mx: "25px", mt: "40px"}} direction={"column"}>
			<CForm  outlinedStyling="neutral" values={{
				name: project.name,
				title: project.title ?? "",
			}} entries={[
				{
			...
			]} onSendEdit={handleModify}  managedButtonPosition="flex-end">
			...
			]} onSendEdit={handleModifyVersion} managedButtonPosition="flex-end">
			...
			<Stack direction={"row"} sx={{alignItems: "center"}}>
				...
				<CChip sx={{ml: "10px"}} label={DProjectLanguages[project.language].display}/>
			</Stack>
